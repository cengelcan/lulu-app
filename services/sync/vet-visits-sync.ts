...
import type { VetVisitBundle, VetVisitQuestion, VetVisitStatus } from '@/types/vet-visit';

type RemoteVisitRow = {
  id: string; user_id: string; pet_id: string; scheduled_at: string;
  provider_id: string | null; provider_name: string | null; reason: string; status: string;
...
function fromRows(row: RemoteVisitRow, questions: RemoteQuestionRow[]): VetVisitBundle {
  return {
    visit: {
...
export async function fetchRemoteVetVisits(): Promise<VetVisitBundle[]> {
  ...
  return (visits as RemoteVisitRow[]).map((visit) =>
    fromRows(visit, questions as RemoteQuestionRow[])
  );
}
