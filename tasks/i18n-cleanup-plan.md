...
- [ ] `stores/pet-record.store.ts`
- [ ] `stores/pet-reminder.store.ts`
...
- [ ] `app/index.tsx` — splash error + `Try Again` → `t('common.tryAgain')`
- [ ] `hooks/use-bootstrap.ts` — `onboardingUnavailable` key
...
- [ ] `services/reports/generate-report-html.ts`
  - [ ] Pass `language` / `lang` attribute (`en` | `de`) instead of hardcoded `lang="en"`
  - [ ] Localize `<title>` suffix (currently `— Report`)
- [ ] `services/reports/export-report-pdf.ts`
  - [ ] Localize `dialogTitle` (`Share report`)
  - [ ] Localize `DEFAULT_REPORT_FILE_NAME` fallback
  - [ ] Localize `'Sharing is not available on this device'` — throw keyed error, translate in UI
- [ ] `services/reports/html/components/report-header.ts`
  - [ ] App Store badge text (`Download on the`, `App Store`) — pass as labels or skip badge in DE for v1
  - [ ] `alt="QR code"` — keyed a11y string (low priority for PDF)
- [ ] `components/reports/ReportsWizardContent.tsx` — pass language into `generateReportHtml` and `exportReportPdf`
